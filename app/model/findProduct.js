...
function findProducto(codeEan, callback) {
  db.getConnection((error, connection) => {
    ...
    const query = `SELECT idProducto, producto, descripcion, precio, cantidad FROM TPRODUCTS WHERE idProducto = ?`;

    connection.query(query, [codeEan], (error, results) => {
      if (error) {
